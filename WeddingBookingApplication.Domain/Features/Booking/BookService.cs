...
public class BookService
{
    ...
    public BookingCreateResponseModel CreateBooking(BookingCreateRequestModel request)
    {
        ...
        {
            // 1. Validate Vendor
            var vendor = _db.Vendors.FirstOrDefault(v => v.VendorId == request.VendorId && v.Status != 0);
            if (vendor == null)
            {
            ...
            // 2. Validate Venue
            var venue = _db.Venues.FirstOrDefault(v => v.VenueId == request.VenueId && v.IsActive);
            if (venue == null)
            {
            ...
            var decorationPackages = new List<DecorationPackage>();
            foreach (var decId in request.DecorationPackageIds)
            {
                var decPkg = _db.DecorationPackages.FirstOrDefault(dp => dp.DecorationPackageId == decId && dp.IsActive);
                if (decPkg == null)
                {
            ...
            var servicePackages = new List<Database.AppDbContextModels.ServicePackage>();
            foreach (var srvId in request.ServicePackageIds)
            {
                var srvPkg = _db.ServicePackages.FirstOrDefault(sp => sp.ServicePackageId == srvId && sp.IsActive);
                if (srvPkg == null)
                {
    ...
    {
        var query = _db.UserBookings
            .Include(b => b.Vendor)
            .Include(b => b.Venue)
            .Include(b => b.BookingDecorations).ThenInclude(bd => bd.DecorationPackage)
            .Include(b => b.BookingServices).ThenInclude(bs => bs.ServicePackage)
            .AsQueryable();
    ...
    public BookingStatusUpdateResponseModel CancelBooking(int bookingId)
    {
        try
        {
            var booking = _db.UserBookings.FirstOrDefault(b => b.BookingId == bookingId);
            if (booking == null)
            {
    ...
    // ── Mapping helper ──────────────────────────────────────────────────────────
    private static BookingResponseModel MapToResponse(UserBooking b)
    {
        return new BookingResponseModel
        {
            ...
            Decorations = b.BookingDecorations
